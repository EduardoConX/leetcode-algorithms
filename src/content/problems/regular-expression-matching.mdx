...
### Algorithm
1. If `p` is empty → return `true` only if `s` is also empty.  
...
### Steps
1. Create a 2D boolean table `dp` of size `(len(s)+1) × (len(p)+1)`.  
...
```text
create matrix dp of size (m+1) × (n+1), filled with false
dp[m][n] = true
